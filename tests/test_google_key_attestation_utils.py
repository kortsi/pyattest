"""Tests for Key Attestation fetch/parse utilities and adversarial inputs."""
...
def test_parse_root_certs_adds_new():
    ...
    cert = (
        ...
        .subject_name(cx509.Name([cx509.NameAttribute(NameOID.COMMON_NAME, "New Root")]))
...
def test_fetch_revocation_delegates_to_parse():
    ...
    fake_response = MagicMock()
    fake_response.read.return_value = json.dumps({"entries": {"abc": {"status": "REVOKED"}}}).encode()
